...
export function handleMarkedAsPubliclyDecryptable(
  event: MarkedAsPubliclyDecryptableEvent
): void {
  let entity = new MarkedAsPubliclyDecryptable(
    event.transaction.hash.concatI32(event.logIndex.toI32())
  )
  entity.sender = event.params.sender
  entity.handle = event.params.handle
...
export function handleNoxComputeUpdated(event: NoxComputeUpdatedEvent): void {
  let entity = new NoxComputeUpdated(
    event.transaction.hash.concatI32(event.logIndex.toI32())
  )
